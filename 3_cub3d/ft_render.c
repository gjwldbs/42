...
void    wall_casting_info_4(t_vars *vars, int h)
{
   ...
   if(vars->side == 0 && vars->raydirX > 0) vars->texX =
    texWidth - vars->texX - 1;
   if(vars->side == 1 && vars->raydirY < 0) vars->texX =
    texWidth - vars->texX - 1;
...
void    render(t_vars *vars)
{
   int x;
   int i;
   int w = vars->pars.resolution_w;
   int h = vars->pars.resolution_h;
